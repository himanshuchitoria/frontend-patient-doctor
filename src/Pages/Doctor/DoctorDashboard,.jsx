...
const DoctorDashboard = () => {
  ...
  const baseUrl = "https://backend-dashboard-v3o0.onrender.com";
  ...
  useEffect(() => {
    ...
    if (doctorId && token) {
      axios
        .get(`${baseUrl}/appointment/doctor/${doctorId}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
  ...
  useEffect(() => {
    ...
    if (doctorId && slotDate) {
      axios
        .get(
          `${baseUrl}/appointment/doctor/slots?doctorId=${doctorId}&date=${slotDate}`,
          { headers: { Authorization: `Bearer ${token}` } }
        )
        .then((res) =>
          setSlots(
  ...
  useEffect(() => {
    ...
    if (doctorId && token) {
      axios
        .get(`${baseUrl}/doctor/${doctorId}/blogs`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
  ...
  const toggleSlotAvailability = async (slotId, available) => {
    try {
      await axios.patch(
        `${baseUrl}/appointment/slot/${slotId}/availability`,
        { available },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setSlots((prev) =>
  ...
  const saveEditedSlot = async (slotId) => {
    ...
    try {
      const response = await axios.patch(
        `${baseUrl}/appointment/slot/${slotId}/timings`,
        editedSlotTimes,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setSlots((prev) =>
  ...
  const generateSlots = async () => {
    ...
    try {
      await axios.post(
        `${baseUrl}/appointment/doctor/${doctor._id}/generate-slots`,
        { date: slotDate },
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
const fetchBlogs = async () => {
    try {
      const doctorId = localStorage.getItem("userId");
      const response = await axios.get(`${baseUrl}/doctor/${doctorId}/blogs`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleBlogInputChange = (field, value) => {
    if (editingBlogId) {
      setEditingBlogData((prev) => ({ ...prev, [field]: value }));
    } else {
  ...
  const submitNewBlog = async () => {
    const doctorId = localStorage.getItem("userId");
    if (!newBlogData.headTitle || !newBlogData.body) {
      toast.error("Title and body are required for blog");
      return;
    }
    try {
      const response = await axios.post(
        `${baseUrl}/doctor/${doctorId}/blogs`,
        newBlogData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success("Blog posted successfully");
      setNewBlogData({ headTitle: "", body: "", image: "" });
      fetchBlogs();
    } catch {
  ...
  const cancelEditBlog = () => {
    ...
    setEditingBlogData({ headTitle: "", body: "", image: "" });
  };

  const submitEditBlog = async () => {
    const doctorId = localStorage.getItem("userId");
    if (!editingBlogData.headTitle || !editingBlogData.body) {
      toast.error("Title and body are required for blog");
      return;
    }
    try {
      await axios.patch(
        `${baseUrl}/doctor/${doctorId}/blogs/${editingBlogId}`,
        editingBlogData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteBlog = async (blogId) => {
    const doctorId = localStorage.getItem("userId");
    try {
      await axios.delete(`${baseUrl}/doctor/${doctorId}/blogs/${blogId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        <header className="main-header-theme">
          <div className="header-content">
            <h1 className="header-title-theme">
              ...
              Welcome, <br/>Dr. {doctor ? `${doctor.firstName} ${doctor.lastName}` : "Loading..."}
            </h1>
        ...
        <div className="main-content-container">
          ...
          <div className="appointments-glass-card">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:gap-6 mb-2">
              ...
              />
              <button className="generate-slots-btn" onClick={generateSlots}>
              ...
              <div className="doctor-info-block">
                {doctor ? (
                  <>
                    ...
                    {/* Similarly other editable fields: lastName, specialty, clinicLocation, contactNumber, workingHours */}
                    {/* Previously provided code for those fields remains unchanged, omitted for brevity */}
          ...
          <div className="appointments-glass-card">
            <h2 className="appointments-title">
              <FontAwesomeIcon icon={faUserMd} className="icon-accent" /> Your Blogs
            </h2>
            ...
            {!editingBlogId && (
              <div className="blog-form">
                <input
                  ...
                  value={newBlogData.headTitle}
                  onChange={(e) => handleBlogInputChange("headTitle", e.target.value)}
                />
                <textarea
                  ...
                  onChange={(e) => handleBlogInputChange("body", e.target.value)}
                />
                <input
                  type="text"
                  ...
                  onChange={(e) => handleBlogInputChange("image", e.target.value)}
                />
                <button className="generate-slots-btn" onClick={submitNewBlog}>
            ...
            {editingBlogId && (
              <div className="blog-form">
                <input
                  ...
                  placeholder="Blog Title"
                  className="profile-input mb-2"
                  value={editingBlogData.headTitle}
                  onChange={(e) => handleBlogInputChange("headTitle", e.target.value)}
                />
                <textarea
                  placeholder="Blog Content"
                ...
                <input
                  ...
                  placeholder="Image URL (optional)"
                  className="profile-input mb-2"
                  value={editingBlogData.image}
                  onChange={(e) => handleBlogInputChange("image", e.target.value)}
                />
                <button className="btn btn-success mr-2" onClick={submitEditBlog}>
                  <FontAwesomeIcon icon={faSave} /> Save
                </button>
                <button className="btn btn-danger" onClick={cancelEditBlog}>
            ...
            {/* Blogs List */}
            <div className="blogs-list mt-4">
              {blogs.length === 0 ? (
                <p>No blogs posted yet.</p>
              ) : (
                blogs.map((blog) => (
                  <div key={blog._id} className="blog-card">
                    {blog.image && <img src={blog.image} alt={blog.headTitle} className="blog-image" />}
                    <div className="blog-content">
                      <h3>{blog.headTitle.length > 30
    ? blog.headTitle.slice(0, 30) + ".."
    : blog.headTitle}</h3>
                      <p className="blog-author-date">
                        By {blog.author} | {new Date(blog.createdAt).toLocaleDateString()}
                      </p>
                     
                      <div className="blog-actions mt-2">
                        <button className="btn btn-primary mr-2" onClick={() => startEditBlog(blog)}>
          ...
          {/* APPOINTMENTS SUMMARY AND TABLE */}
          <div className="appointments-glass-card">
            <h2 className="appointments-title">
